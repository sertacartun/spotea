...
The API has no GET (/settings is the Settings page), so an empty PUT reads the current values.
"""
...
def test_new_downloads_default_to_data_saver(client):
    ...
    assert client.put("/settings", json={}).json()["audio_quality"] == "low"
...
def test_changing_the_quality_is_reflected_immediately(client):
    ...
    assert client.put("/settings", json={}).json()["audio_quality"] == "high"
...
def test_a_different_user_is_unaffected(client):
    ...
    with client.__class__(client.app) as other:
        ...
        assert other.put("/settings", json={}).json()["audio_quality"] == "low"
...
def test_the_settings_payload_no_longer_carries_a_refresh_interval(client):
    ...
    assert "refresh_interval_minutes" not in client.put("/settings", json={}).json()
